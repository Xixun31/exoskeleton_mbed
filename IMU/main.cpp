...
char myRxData[6] = {0};
// IMU
MTi2Class IMU(1000000, PB_15, PB_14, PB_13,
              PB_1); // Create MTi2 Class (Frequency, MOSI, MISO, SCLK, CS)
...
int main() {
  ...
  pc.printf("init_IO...\r\n");
  init_IO();
  pc.printf("init_TIMER...\r\n");
  init_TIMER();
  pc.printf("init_SPI...\r\n");
  init_SPI();
  pc.printf("init_UART...\r\n");
  init_UART();
  pc.printf("IMU.MTi2_Init...\r\n");
  IMU.MTi2_Init();
  pc.printf("Entering main loop...\r\n");
  while (true) {
      wait_us(50000); // 50ms wait directly instead of Ticker for now
      read_position();
      UART_TX();
  }
}

void init_IO() { encoder_cs = 1; }
void init_TIMER() {
//  timer1.attach_us(
//      &timer1_ITR,
//      50000.0); // the address of the function to be attached (timer1_ITR) and
                // the interval ``1000 micro-seconds)
}
void init_SPI() {
  spi.format(16, 3);
...
void UART_TX() {
  ...
  //    pc.printf("%.5f,\n\r%.5f,\n\r%.2f,\n\r%.2f,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%.3f,
  //    %.3f,
  //    %.3f\n\r",v_L,v_H,ss,init_position,angle,angle2,angle3,angle4,angle5,angle6,angle7,angle8,angle9,angle10,angle11,
  //    pitch, roll, yaw);
  // pc.printf("r%d,\n\r",init_position);
  ///    pc.printf("%.5f,\n\r%.5f,\n\r%.2f,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r",v_L,v_H,ss,init_position,angle,angle2,angle3,angle4,angle5,angle6,angle7,angle8,angle9,angle10,angle11);//common
  ///    used
  pc.printf("%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,"
            "\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%d,\n\r%.2f,\n\r%.2f,\n\r%.2f,"
            "\n\r%.2f,\n\r%.2f,\n\r%.2f,\n\r%.2f,\n\r%.2f,\n\r%.2f,\n\r",
            O_ang1, O_ang2, O_len, angle, angle2, angle3, angle4, angle5,
            angle6, angle7, angle8, angle9, angle10, angle11, angle12,
            IMU.euler[0], IMU.euler[1], IMU.euler[2], IMU.omega[0],
            IMU.omega[1], IMU.omega[2], IMU.accel[0], IMU.accel[1],
            IMU.accel[2]); // COE used
